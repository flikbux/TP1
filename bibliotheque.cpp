...
std::vector<Auteur> Bibliotheque::GetAuteurList() {
    return _auteur_list;
}

std::vector<Lecteur> Bibliotheque::GetLecteurList() {
    return _lecteur_list;
}
...
void Bibliotheque::AddLecteur(Lecteur& lecteur) {
    ...
    if (check == _lecteur_id_list.end()){
        _lecteur_list.emplace_back(lecteur);
        _lecteur_id_list.emplace_back(lecteur.GetIdentifiant());
...
void Bibliotheque::AddLivre(Livre& livre) {
    ...
    if (check == _livre_ISBN_list.end()){
        _livre_list.emplace_back(livre);
        _livre_ISBN_list.emplace_back(livre.GetISBN());
...
void Bibliotheque::AddAuteur(Auteur& auteur) {
    ...
    if (check == _auteur_id_list.end()){
        _auteur_list.emplace_back(auteur);
        _auteur_id_list.emplace_back(auteur.GetIdentifiant());
